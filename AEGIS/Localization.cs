...
namespace AEGIS
{
    ...
    public static class Loc
    {
        ...
        private static readonly Dictionary<string, string> DeStrings = new()
        {
            ...
            ["SuiteBuilder.Description"] = "Lädt die neueste Version eines Kpmn-Sticks von GitHub und schreibt sie auf einen angeschlossenen Wechseldatenträger.",
            ...
            ["SuiteBuilder.Mabs.CardDescription"] = "Installiert Ventoy auf dem gewählten Datenträger und kopiert anschließend das Kpmn-Theme darauf. Ventoy wird dabei direkt von seiner offiziellen GitHub-Seite geladen. ISO-Dateien gehören nicht zum Repository – Ubuntu und Kali kann AEGIS danach auf Wunsch direkt von den offiziellen Servern der Projekte holen, alle anderen Images kommen von Hand auf den Stick.",
            ...
            ["SuiteBuilder.Mabs.StageSettingLabel"] = "Setze die Laufwerksbezeichnung auf „MABS“…",
            ["SuiteBuilder.Mabs.LabelFailed"] = "Achtung: Laufwerk konnte nicht in „MABS“ umbenannt werden: {0}",
            ...
            ["SuiteBuilder.Mabs.IsoHint"] = "Hinweis: ISO-Dateien gehören bewusst nicht zum Repository – Kpmn Development verteilt keine Betriebssystem-Images. Die gewünschten ISOs gehören in die Wurzel der Ventoy-Datenpartition; Ventoy nimmt sie von allein ins Bootmenü auf.",
            ...
            // Suite-Builder: Betriebssystem-Images nach einem MABS-Build auf die Ventoy-Datenpartition laden
            ["SuiteBuilder.Mabs.Iso.Title"] = "Betriebssystem-Images hinzufügen",
            ["SuiteBuilder.Mabs.Iso.Intro"] = "Der Stick ist fertig, das Ventoy-Bootmenü aber noch leer: Ventoy startet jede ISO-Datei, die in der Wurzel der Datenpartition liegt, ganz von allein.\n\nUbuntu und Kali lädt AEGIS auf Wunsch in der jeweils aktuellen Version direkt von den offiziellen Servern der Projekte herunter und legt sie dort ab. Kpmn Development hostet oder verteilt selbst keine Betriebssystem-Images.\n\nBeide Dateien sind mehrere Gigabyte groß, der Download dauert entsprechend lange. Dieser Schritt ist optional – der Stick funktioniert bereits, weitere ISOs lassen sich jederzeit selbst hineinkopieren. Das Schließen dieses Fensters bricht einen laufenden Download ab.",
            ["SuiteBuilder.Mabs.Iso.TargetRoot"] = "Ziel: {0} (noch {1} frei)",
            ["SuiteBuilder.Mabs.Iso.UbuntuDescription"] = "Aktuelle LTS-Version, Desktop, 64 Bit (~6 GB)",
            ["SuiteBuilder.Mabs.Iso.KaliDescription"] = "Aktuelle Version, Installer-Image, 64 Bit (~5 GB)",
            ["SuiteBuilder.Mabs.Iso.SourcesNote"] = "Quellen: releases.ubuntu.com (Ubuntu) und cdimage.kali.org (Kali Linux) – jeweils die offiziellen Server der Projekte.",
            ["SuiteBuilder.Mabs.Iso.DownloadButton"] = "Herunterladen",
            ["SuiteBuilder.Mabs.Iso.Close"] = "Schließen",
            ["SuiteBuilder.Mabs.Iso.StatePending"] = "nicht geladen",
            ["SuiteBuilder.Mabs.Iso.StateDownloading"] = "lädt… {0} %",
            ["SuiteBuilder.Mabs.Iso.StateDone"] = "fertig",
            ["SuiteBuilder.Mabs.Iso.StateAlreadyPresent"] = "liegt bereits auf dem Stick",
            ["SuiteBuilder.Mabs.Iso.StateFailed"] = "fehlgeschlagen: {0}",
            ["SuiteBuilder.Mabs.Iso.StateCancelled"] = "abgebrochen",
            ["SuiteBuilder.Mabs.Iso.Progress"] = "Lade {0}… {1} % ({2} / {3})",
            ["SuiteBuilder.Mabs.Iso.ProgressUnknownSize"] = "Lade {0}… {1}",
            ["SuiteBuilder.Mabs.Iso.SummaryOk"] = "Fertig. „{0}“ liegt jetzt auf {1} und erscheint beim nächsten Start im Ventoy-Menü.",
            ["SuiteBuilder.Mabs.Iso.SummaryAlreadyPresent"] = "„{0}“ liegt bereits auf {1} – es wurde nichts erneut geladen.",
            ["SuiteBuilder.Mabs.Iso.SummaryFailed"] = "{0} konnte nicht geladen werden: {1}",
            ["SuiteBuilder.Mabs.Iso.SummaryCancelled"] = "Abgebrochen – {0} wurde nicht vollständig geladen und die unvollständige Datei wieder entfernt.",
            ["SuiteBuilder.Mabs.Iso.ManualHeading"] = "Windows musst du selbst herunterladen",
            ["SuiteBuilder.Mabs.Iso.ManualHint"] = "Ein Windows-11-Image lässt sich nicht automatisch laden: Microsoft erzeugt die eigentliche Download-Adresse erst nach einer Auswahl im Browser, bindet sie an die Sitzung und lässt sie nach 24 Stunden ablaufen. Lade die ISO-Datei deshalb selbst über Microsofts offizielle Seite herunter und kopiere sie anschließend einfach in die Wurzel der Datenpartition – Ventoy nimmt sie dann von allein ins Bootmenü auf.",
            ["SuiteBuilder.Mabs.Iso.StatusDownloading"] = "~/aegis $  Lade {0} nach {1}…",
            ["SuiteBuilder.Mabs.Iso.StatusDone"] = "~/aegis $  {0} nach {1} geladen",
            ["SuiteBuilder.Mabs.Iso.StatusAlreadyPresent"] = "~/aegis $  {0} liegt bereits auf dem Stick",
            ["SuiteBuilder.Mabs.Iso.StatusFailed"] = "~/aegis $  {0} konnte nicht geladen werden: {1}",
            ["SuiteBuilder.Mabs.Iso.StatusCancelled"] = "~/aegis $  Download von {0} abgebrochen",
            ["SuiteBuilder.Mabs.Iso.ErrorNoUrl"] = "Keine Download-Adresse gefunden.",
            ["SuiteBuilder.Mabs.Iso.ErrorUbuntuNoRelease"] = "Auf releases.ubuntu.com wurde keine LTS-Version gefunden.",
            ["SuiteBuilder.Mabs.Iso.ErrorUbuntuNoIso"] = "Im Ubuntu-Verzeichnis wurde kein Desktop-Image für 64 Bit gefunden.",
            ["SuiteBuilder.Mabs.Iso.ErrorKaliNoIso"] = "Im Kali-Verzeichnis wurde kein Installer-Image für 64 Bit gefunden.",
            ...
            // Suite-Builder: Treiber-Scanner (SDIO) – wird in beiden Varianten immer mitgeladen
            ["DriverSetup.SdiToolHeading"] = "Treiber-Scanner (SDIO)",
            ["DriverSetup.SdiToolDescription"] = "Das eigentliche Scanner-Programm, das AVAS auf dem Ziel-PC startet: \"Snappy Driver Installer Origin\", der gepflegte Nachfolger des eingestellten SDI. Ohne dieses Programm im Ordner \"Drivers\" überspringt AVAS den Treiberschritt kommentarlos. AEGIS lädt es direkt von der offiziellen Seite des Entwicklers – unabhängig davon, welche Treiber-Datenbank du wählst.",
            ["DriverSetup.SdiToolResolving"] = "Suche aktuelle SDIO-Version…",
            ["DriverSetup.SdiToolDownloading"] = "Lade SDIO… {0} % ({1} / {2})",
            ["DriverSetup.SdiToolDownloadingUnknownSize"] = "Lade SDIO… {0}",
            ["DriverSetup.SdiToolExtracting"] = "Entpacke SDIO…",
            ["DriverSetup.SdiToolDone"] = "Fertig. \"{0}\" liegt jetzt im Ordner \"{1}\".",
            ["DriverSetup.SdiToolAlreadyPresent"] = "Bereits vorhanden: \"{0}\" liegt schon im Ordner \"Drivers\".",
            ["DriverSetup.SdiToolRetryButton"] = "SDIO erneut herunterladen",
            ["DriverSetup.SdiToolCancelled"] = "SDIO-Download abgebrochen.",
            ["DriverSetup.SdiToolError"] = "SDIO konnte nicht geladen werden: {0} – die restliche Treiber-Einrichtung funktioniert trotzdem, du kannst es später erneut versuchen.",
            ["DriverSetup.SdiToolNoLinkFound"] = "Auf der offiziellen SDIO-Seite wurde kein Download-Link gefunden.",
            ["DriverSetup.SdiToolNoExeFound"] = "Im SDIO-Archiv wurde kein 64-Bit-Hauptprogramm (SDIO_x64_R*.exe) gefunden.",
            ["DriverSetup.StatusSdiTool"] = "~/aegis $  Treiber-Scanner \"{0}\" nach Drivers kopiert",

            // Suite-Builder: Software-Pakete nach einem AVAS-Build in den Ordner "Files" laden
            ["PackageSetup.Title"] = "Software-Pakete herunterladen",
            ["PackageSetup.Intro"] = "AVAS installiert auf dem Ziel-PC sieben Standard-Programme: Mozilla Firefox, Google Chrome, Brave Browser, 7-Zip, VLC Media Player, Notepad++ und Visual Studio Code. Die passenden Installationsdateien müssen dafür im Ordner \"Files\" auf dem Stick liegen.\n\nAEGIS lädt sie auf Wunsch in der jeweils aktuellen Version direkt von den offiziellen Seiten der Hersteller. Kpmn Development hostet oder verteilt selbst keine fremde Software.\n\nDieser Schritt ist optional – fehlende Dateien überspringt AVAS später einfach.",
            ["PackageSetup.BraveStubHint"] = "Brave stellt nur einen Online-Installer bereit: Die geladene Datei ist mit rund 1,3 MB nur ein kleines Startprogramm, das die eigentlichen Browser-Dateien erst während der Installation aus dem Internet nachlädt. Die anderen sechs Pakete sind vollständige Offline-Installer. Auf einem Ziel-PC ohne Internetverbindung lässt sich Brave von diesem Stick daher nicht fertig installieren.",
            ["PackageSetup.ChromeNote"] = "Hinweis: Google bietet die für eine unbeaufsichtigte Installation geeignete Chrome-Version nur als MSI-Paket an, daher heißt die Datei \"chrome_setup.msi\" statt \".exe\".",
            ["PackageSetup.DownloadAllButton"] = "Alle Pakete herunterladen",
            ["PackageSetup.Close"] = "Schließen",
            ["PackageSetup.FolderCreated"] = "Zielordner: {0}",
            ["PackageSetup.FolderError"] = "Ordner \"{0}\" konnte nicht angelegt werden: {1}",
            ["PackageSetup.StatePending"] = "ausstehend",
            ["PackageSetup.StateDownloading"] = "lädt… {0} %",
            ["PackageSetup.StateDone"] = "fertig",
            ["PackageSetup.StateFailed"] = "fehlgeschlagen: {0}",
            ["PackageSetup.StateCancelled"] = "abgebrochen",
            ["PackageSetup.Progress"] = "Lade {0} ({1} von {2})… {3} % ({4} / {5})",
            ["PackageSetup.ProgressUnknownSize"] = "Lade {0} ({1} von {2})… {3}",
            ["PackageSetup.FailureItem"] = "{0} ({1})",
            ["PackageSetup.SummaryAllOk"] = "Fertig. Alle {0} Pakete liegen jetzt in \"{1}\".",
            ["PackageSetup.SummaryPartial"] = "{0} von {1} erfolgreich. Fehlgeschlagen: {2}",
            ["PackageSetup.SummaryCancelled"] = "Abgebrochen – {0} von {1} Paketen wurden geladen.",
            ["PackageSetup.StatusDownloading"] = "~/aegis $  Lade Software-Pakete nach {0}…",
            ["PackageSetup.StatusFolderCreated"] = "~/aegis $  Ordner für Software-Pakete angelegt: {0}",
            ["PackageSetup.StatusDone"] = "~/aegis $  Software-Pakete: {0}/{1} nach {2} geladen",
            ["PackageSetup.StatusCancelled"] = "~/aegis $  Download der Software-Pakete abgebrochen ({0}/{1})",

            // Suite-Builder: portable Diagnose-Tools nach einem DART-Build auf den Stick laden
            ["DartToolSetup.Title"] = "Diagnose-Tools herunterladen",
            ["DartToolSetup.Intro"] = "DART startet portable Diagnose-Programme vom Stick, die selbst nicht zum DART-Repository gehören – ohne sie melden die Schaltflächen nur \"Datei nicht gefunden\".\n\nFünf davon lädt AEGIS auf Wunsch in der jeweils aktuellen Version direkt von den offiziellen Quellen und legt sie in den festen Ordnern ab, die DART erwartet: CPU-Z, WifiChannelMonitor, System Informer, die Sysinternals Suite und TreeSize Free. Kpmn Development hostet oder verteilt selbst keine fremde Software.\n\nDieser Schritt ist optional und kann jederzeit abgebrochen werden.",
            ["DartToolSetup.TargetRoot"] = "Ziel-Stick: {0}",
            ["DartToolSetup.SourcesNote"] = "Quellen: cpuid.com (CPU-Z), nirsoft.net (WifiChannelMonitor), GitHub-Release winsiderss/systeminformer (System Informer), download.sysinternals.com (Sysinternals Suite) und jam-software.de (TreeSize Free).",
            ["DartToolSetup.ManualHeading"] = "Zwei Tools musst du selbst herunterladen",
            ["DartToolSetup.ManualHint"] = "HWiNFO und CrystalDiskInfo lassen sich nicht automatisch laden: Ihre Downloadseiten sind durch eine Bot-Prüfung geschützt, die Anfragen von AEGIS blockiert (im Browser funktionieren sie normal). Lade die beiden Archive daher selbst herunter und entpacke sie in die angegebenen Ordner auf dem Stick – die Ordner ggf. vorher anlegen.",
            ["DartToolSetup.ManualHwInfoLabel"] = "HWiNFO – \"HWiNFO64.exe\" und \"HWiNFO32.exe\" aus dem portablen Archiv nach: {0}",
            ["DartToolSetup.ManualCrystalDiskInfoLabel"] = "CrystalDiskInfo – \"DiskInfo64.exe\" samt restlichem Inhalt des Archivs nach: {0}",
            ["DartToolSetup.DownloadAllButton"] = "Alle Tools herunterladen",
            ["DartToolSetup.Close"] = "Schließen",
            ["DartToolSetup.StatePending"] = "ausstehend",
            ["DartToolSetup.StateDownloading"] = "lädt… {0} %",
            ["DartToolSetup.StateDone"] = "fertig",
            ["DartToolSetup.StateFailed"] = "fehlgeschlagen: {0}",
            ["DartToolSetup.StateCancelled"] = "abgebrochen",
            ["DartToolSetup.Progress"] = "Lade {0} ({1} von {2})… {3} % ({4} / {5})",
            ["DartToolSetup.ProgressUnknownSize"] = "Lade {0} ({1} von {2})… {3}",
            ["DartToolSetup.FailureItem"] = "{0} ({1})",
            ["DartToolSetup.SummaryAllOk"] = "Fertig. Alle {0} Tools liegen jetzt auf {1}.",
            ["DartToolSetup.SummaryPartial"] = "{0} von {1} erfolgreich. Fehlgeschlagen: {2}",
            ["DartToolSetup.SummaryCancelled"] = "Abgebrochen – {0} von {1} Tools wurden geladen.",
            ["DartToolSetup.StatusDownloading"] = "~/aegis $  Lade Diagnose-Tools nach {0}…",
            ["DartToolSetup.StatusDone"] = "~/aegis $  Diagnose-Tools: {0}/{1} nach {2} geladen",
            ["DartToolSetup.StatusCancelled"] = "~/aegis $  Download der Diagnose-Tools abgebrochen ({0}/{1})",
            ["DartToolSetup.ErrorNoUrl"] = "Keine Download-Adresse gefunden.",
            ["DartToolSetup.ErrorNoMatchingFiles"] = "Im heruntergeladenen Archiv wurden die erwarteten Programmdateien nicht gefunden.",
            ["DartToolSetup.ErrorCpuZNoZip"] = "Auf der CPU-Z-Seite wurde kein englisches ZIP-Paket gefunden.",
        ...
        private static readonly Dictionary<string, string> EnStrings = new()
        {
            ...
            // Suite Builder tab
            ["SuiteBuilder.Header"] = "SUITE BUILDER",
            ["SuiteBuilder.Description"] = "Downloads the latest version of a Kpmn stick from GitHub and writes it to a connected removable drive.",
            ...
            ["SuiteBuilder.Mabs.CardDescription"] = "Installs Ventoy on the selected drive and then copies the Kpmn theme onto it. Ventoy is downloaded directly from its official GitHub page. ISO files are not part of the repository — afterwards AEGIS can fetch Ubuntu and Kali straight from the projects' official servers on request; any other image has to be added by hand.",
            ...
            ["SuiteBuilder.Mabs.StageSettingLabel"] = "Setting the volume label to \"MABS\"…",
            ["SuiteBuilder.Mabs.LabelFailed"] = "Warning: the drive could not be renamed to \"MABS\": {0}",
            ...
            ["SuiteBuilder.Mabs.IsoHint"] = "Note: ISO files are deliberately not part of the repository — Kpmn Development does not distribute operating system images. The ISOs you want belong in the root of the Ventoy data partition; Ventoy adds them to the boot menu on its own.",
            ...
            // Suite Builder: download operating system images onto the Ventoy data partition after a MABS build
            ["SuiteBuilder.Mabs.Iso.Title"] = "Add operating system images",
            ["SuiteBuilder.Mabs.Iso.Intro"] = "The stick is finished, but the Ventoy boot menu is still empty: Ventoy boots any ISO file placed in the root of the data partition all by itself.\n\nOn request, AEGIS downloads Ubuntu and Kali in their current version straight from the projects' official servers and places them there. Kpmn Development does not host or redistribute any operating system images itself.\n\nBoth files are several gigabytes in size, so the download takes a while. This step is optional — the stick already works, and further ISOs can be copied in by hand at any time. Closing this window cancels a running download.",
            ["SuiteBuilder.Mabs.Iso.TargetRoot"] = "Target: {0} ({1} free)",
            ["SuiteBuilder.Mabs.Iso.UbuntuDescription"] = "Current LTS version, desktop, 64-bit (~6 GB)",
            ["SuiteBuilder.Mabs.Iso.KaliDescription"] = "Current version, installer image, 64-bit (~5 GB)",
            ["SuiteBuilder.Mabs.Iso.SourcesNote"] = "Sources: releases.ubuntu.com (Ubuntu) and cdimage.kali.org (Kali Linux) — the projects' own official servers.",
            ["SuiteBuilder.Mabs.Iso.DownloadButton"] = "Download",
            ["SuiteBuilder.Mabs.Iso.Close"] = "Close",
            ["SuiteBuilder.Mabs.Iso.StatePending"] = "not downloaded",
            ["SuiteBuilder.Mabs.Iso.StateDownloading"] = "downloading… {0} %",
            ["SuiteBuilder.Mabs.Iso.StateDone"] = "done",
            ["SuiteBuilder.Mabs.Iso.StateAlreadyPresent"] = "already on the stick",
            ["SuiteBuilder.Mabs.Iso.StateFailed"] = "failed: {0}",
            ["SuiteBuilder.Mabs.Iso.StateCancelled"] = "cancelled",
            ["SuiteBuilder.Mabs.Iso.Progress"] = "Downloading {0}… {1} % ({2} / {3})",
            ["SuiteBuilder.Mabs.Iso.ProgressUnknownSize"] = "Downloading {0}… {1}",
            ["SuiteBuilder.Mabs.Iso.SummaryOk"] = "Done. \"{0}\" is now on {1} and will show up in the Ventoy menu on the next boot.",
            ["SuiteBuilder.Mabs.Iso.SummaryAlreadyPresent"] = "\"{0}\" is already on {1} — nothing was downloaded again.",
            ["SuiteBuilder.Mabs.Iso.SummaryFailed"] = "{0} could not be downloaded: {1}",
            ["SuiteBuilder.Mabs.Iso.SummaryCancelled"] = "Cancelled — {0} was not downloaded completely and the incomplete file was removed again.",
            ["SuiteBuilder.Mabs.Iso.ManualHeading"] = "Windows has to be downloaded by hand",
            ["SuiteBuilder.Mabs.Iso.ManualHint"] = "A Windows 11 image cannot be downloaded automatically: Microsoft only generates the actual download address after a selection in the browser, ties it to that session and lets it expire after 24 hours. Please download the ISO file yourself from Microsoft's official page and then simply copy it into the root of the data partition — Ventoy will pick it up for the boot menu on its own.",
            ["SuiteBuilder.Mabs.Iso.StatusDownloading"] = "~/aegis $  Downloading {0} to {1}…",
            ["SuiteBuilder.Mabs.Iso.StatusDone"] = "~/aegis $  {0} downloaded to {1}",
            ["SuiteBuilder.Mabs.Iso.StatusAlreadyPresent"] = "~/aegis $  {0} is already on the stick",
            ["SuiteBuilder.Mabs.Iso.StatusFailed"] = "~/aegis $  {0} could not be downloaded: {1}",
            ["SuiteBuilder.Mabs.Iso.StatusCancelled"] = "~/aegis $  Download of {0} cancelled",
            ["SuiteBuilder.Mabs.Iso.ErrorNoUrl"] = "No download address found.",
            ["SuiteBuilder.Mabs.Iso.ErrorUbuntuNoRelease"] = "No LTS version was found on releases.ubuntu.com.",
            ["SuiteBuilder.Mabs.Iso.ErrorUbuntuNoIso"] = "No 64-bit desktop image was found in the Ubuntu directory.",
            ["SuiteBuilder.Mabs.Iso.ErrorKaliNoIso"] = "No 64-bit installer image was found in the Kali directory.",
            ...
            // Suite builder: driver scanner (SDIO) — always fetched, in both variants
            ["DriverSetup.SdiToolHeading"] = "Driver scanner (SDIO)",
            ["DriverSetup.SdiToolDescription"] = "The actual scanner program AVAS launches on the target PC: \"Snappy Driver Installer Origin\", the maintained successor of the discontinued SDI. Without this program in the \"Drivers\" folder, AVAS silently skips the driver step. AEGIS downloads it straight from the developer's official page — no matter which driver database you choose.",
            ["DriverSetup.SdiToolResolving"] = "Looking up the current SDIO version…",
            ["DriverSetup.SdiToolDownloading"] = "Downloading SDIO… {0} % ({1} / {2})",
            ["DriverSetup.SdiToolDownloadingUnknownSize"] = "Downloading SDIO… {0}",
            ["DriverSetup.SdiToolExtracting"] = "Extracting SDIO…",
            ["DriverSetup.SdiToolDone"] = "Done. \"{0}\" is now in the \"{1}\" folder.",
            ["DriverSetup.SdiToolAlreadyPresent"] = "Already present: \"{0}\" is already in the \"Drivers\" folder.",
            ["DriverSetup.SdiToolRetryButton"] = "Download SDIO again",
            ["DriverSetup.SdiToolCancelled"] = "SDIO download cancelled.",
            ["DriverSetup.SdiToolError"] = "SDIO could not be downloaded: {0} — the rest of the driver setup still works, you can retry later.",
            ["DriverSetup.SdiToolNoLinkFound"] = "No download link was found on the official SDIO page.",
            ["DriverSetup.SdiToolNoExeFound"] = "No 64-bit main program (SDIO_x64_R*.exe) was found in the SDIO archive.",
            ["DriverSetup.StatusSdiTool"] = "~/aegis $  Driver scanner \"{0}\" copied to Drivers",

            // Suite Builder: download the software packages into the "Files" folder after an AVAS build
            ["PackageSetup.Title"] = "Download software packages",
            ["PackageSetup.Intro"] = "AVAS installs seven everyday programs on the target PC: Mozilla Firefox, Google Chrome, Brave Browser, 7-Zip, VLC Media Player, Notepad++ and Visual Studio Code. Their installers have to be present in the \"Files\" folder on the stick for that to work.\n\nOn request, AEGIS downloads the current version of each one straight from the vendor's official site. Kpmn Development does not host or redistribute any third-party software itself.\n\nThis step is optional — AVAS simply skips files that are missing.",
            ["PackageSetup.BraveStubHint"] = "Brave only provides an online installer: the downloaded file is a small stub of about 1.3 MB that pulls the actual browser files from the internet during installation. The other six packages are complete offline installers. On a target PC without an internet connection, Brave therefore cannot be installed from this stick.",
            ["PackageSetup.ChromeNote"] = "Note: Google only offers the Chrome build suitable for unattended installation as an MSI package, so the file is named \"chrome_setup.msi\" instead of \".exe\".",
            ["PackageSetup.DownloadAllButton"] = "Download all packages",
            ["PackageSetup.Close"] = "Close",
            ["PackageSetup.FolderCreated"] = "Target folder: {0}",
            ["PackageSetup.FolderError"] = "Folder \"{0}\" could not be created: {1}",
            ["PackageSetup.StatePending"] = "pending",
            ["PackageSetup.StateDownloading"] = "downloading… {0} %",
            ["PackageSetup.StateDone"] = "done",
            ["PackageSetup.StateFailed"] = "failed: {0}",
            ["PackageSetup.StateCancelled"] = "cancelled",
            ["PackageSetup.Progress"] = "Downloading {0} ({1} of {2})… {3} % ({4} / {5})",
            ["PackageSetup.ProgressUnknownSize"] = "Downloading {0} ({1} of {2})… {3}",
            ["PackageSetup.FailureItem"] = "{0} ({1})",
            ["PackageSetup.SummaryAllOk"] = "Done. All {0} packages are now in \"{1}\".",
            ["PackageSetup.SummaryPartial"] = "{0} of {1} succeeded. Failed: {2}",
            ["PackageSetup.SummaryCancelled"] = "Cancelled — {0} of {1} packages were downloaded.",
            ["PackageSetup.StatusDownloading"] = "~/aegis $  Downloading software packages to {0}…",
            ["PackageSetup.StatusFolderCreated"] = "~/aegis $  Folder for software packages created: {0}",
            ["PackageSetup.StatusDone"] = "~/aegis $  Software packages: {0}/{1} downloaded to {2}",
            ["PackageSetup.StatusCancelled"] = "~/aegis $  Software package download cancelled ({0}/{1})",

            // Suite Builder: download the portable diagnostic tools onto the stick after a DART build
            ["DartToolSetup.Title"] = "Download diagnostic tools",
            ["DartToolSetup.Intro"] = "DART launches portable diagnostic programs from the stick that are not part of the DART repository themselves — without them the buttons only report \"file not found\".\n\nOn request, AEGIS downloads five of them in their current version straight from their official sources and places them in the fixed folders DART expects: CPU-Z, WifiChannelMonitor, System Informer, the Sysinternals Suite and TreeSize Free. Kpmn Development does not host or redistribute any third-party software itself.\n\nThis step is optional and can be cancelled at any time.",
            ["DartToolSetup.TargetRoot"] = "Target stick: {0}",
            ["DartToolSetup.SourcesNote"] = "Sources: cpuid.com (CPU-Z), nirsoft.net (WifiChannelMonitor), the GitHub release of winsiderss/systeminformer (System Informer), download.sysinternals.com (Sysinternals Suite) and jam-software.de (TreeSize Free).",
            ["DartToolSetup.ManualHeading"] = "Two tools have to be downloaded by hand",
            ["DartToolSetup.ManualHint"] = "HWiNFO and CrystalDiskInfo cannot be downloaded automatically: their download pages sit behind a bot check that blocks requests coming from AEGIS (in a browser they work normally). Please download both archives yourself and extract them into the folders listed below — create the folders first if they do not exist yet.",
            ["DartToolSetup.ManualHwInfoLabel"] = "HWiNFO — put \"HWiNFO64.exe\" and \"HWiNFO32.exe\" from the portable archive into: {0}",
            ["DartToolSetup.ManualCrystalDiskInfoLabel"] = "CrystalDiskInfo — put \"DiskInfo64.exe\" and the rest of the archive contents into: {0}",
            ["DartToolSetup.DownloadAllButton"] = "Download all tools",
            ["DartToolSetup.Close"] = "Close",
            ["DartToolSetup.StatePending"] = "pending",
            ["DartToolSetup.StateDownloading"] = "downloading… {0} %",
            ["DartToolSetup.StateDone"] = "done",
            ["DartToolSetup.StateFailed"] = "failed: {0}",
            ["DartToolSetup.StateCancelled"] = "cancelled",
            ["DartToolSetup.Progress"] = "Downloading {0} ({1} of {2})… {3} % ({4} / {5})",
            ["DartToolSetup.ProgressUnknownSize"] = "Downloading {0} ({1} of {2})… {3}",
            ["DartToolSetup.FailureItem"] = "{0} ({1})",
            ["DartToolSetup.SummaryAllOk"] = "Done. All {0} tools are now on {1}.",
            ["DartToolSetup.SummaryPartial"] = "{0} of {1} succeeded. Failed: {2}",
            ["DartToolSetup.SummaryCancelled"] = "Cancelled — {0} of {1} tools were downloaded.",
            ["DartToolSetup.StatusDownloading"] = "~/aegis $  Downloading diagnostic tools to {0}…",
            ["DartToolSetup.StatusDone"] = "~/aegis $  Diagnostic tools: {0}/{1} downloaded to {2}",
            ["DartToolSetup.StatusCancelled"] = "~/aegis $  Diagnostic tool download cancelled ({0}/{1})",
            ["DartToolSetup.ErrorNoUrl"] = "No download address found.",
            ["DartToolSetup.ErrorNoMatchingFiles"] = "The expected program files were not found in the downloaded archive.",
            ["DartToolSetup.ErrorCpuZNoZip"] = "No English ZIP package was found on the CPU-Z page.",
